...
const MEASUREMENT_PRESETS = {
  baby: {
    label: "Baby Wear (0-24 Months)",
    fields: [
      { key: "chest", label: "Chest (বুক)", placeholder: "e.g. 16" },
      { key: "long", label: "Length (দৈর্ঘ্য)", placeholder: "e.g. 14" },
    ],
  },
  tops: {
    label: "Kids Tops (Shirt / Panjabi / Polo / T-Shirt)",
    fields: [
      { key: "chest", label: "Chest (বুক)", placeholder: "e.g. 20" },
      { key: "long", label: "Length / Long (দৈর্ঘ্য)", placeholder: "e.g. 16" },
      { key: "shoulder", label: "Shoulder (কাধ)", placeholder: "e.g. 9" },
      { key: "sleeve", label: "Sleeve (হাতা)", placeholder: "e.g. 10" },
    ],
  },
  bottoms: {
    label: "Kids Bottoms (Pant / Pajama / Trouser)",
    fields: [
      { key: "waist", label: "Waist (কোমর)", placeholder: "e.g. 18" },
      { key: "long", label: "Length / Long (দৈর্ঘ্য)", placeholder: "e.g. 18" },
      { key: "hip", label: "Hip (হিপ)", placeholder: "e.g. 20" },
      { key: "thigh", label: "Thigh (রান)", placeholder: "e.g. 10" },
    ],
  },
  shoes: {
    label: "Kids Shoes / Footwear",
    fields: [
      { key: "footLength", label: "Foot Length (পায়ের দৈর্ঘ্য)", placeholder: "e.g. 12 cm" },
      { key: "euSize", label: "EU/UK Size", placeholder: "e.g. EU 19" },
    ],
  },
  standard: {
    label: "Standard Kids (Long & Body)",
    fields: [
      { key: "long", label: "Long (দৈর্ঘ্য)", placeholder: "e.g. 16" },
      { key: "body", label: "Body / Chest (বুক)", placeholder: "e.g. 20" },
    ],
  },
};

function getPresetFromCategorySlug(slug = "") {
  if (!slug) return "baby";
  const s = slug.toLowerCase();
  if (s.includes("shoe") || s.includes("footwear") || s.includes("slipper") || s.includes("sandal")) return "shoes";
  if (s.includes("pant") || s.includes("pajama") || s.includes("trouser") || s.includes("bottom") || s.includes("jeans") || s.includes("shorts") || s.includes("legging")) return "bottoms";
  if (s.includes("baby") || s.includes("infant") || s.includes("newborn") || s.includes("romper") || s.includes("bodysuit") || s.includes("onesie")) return "baby";
  return "tops";
}
...
export default function AdminProducts({ children }) {
  ...
  const [measurementPreset, setMeasurementPreset] = useState("baby");
  ...
  useEffect(() => {
    if (selectedCategorySlug) {
      setMeasurementPreset(getPresetFromCategorySlug(selectedCategorySlug));
    }
                  ...
                  {!selectedCategorySlug ? (
                    ...
                    <div className="space-y-5">
                      ...
                      {categoryVariants.length > 0 && (
                        <div className="space-y-4 rounded-xl border border-pink-200 p-4 bg-pink-50/40 dark:bg-pink-950/20 dark:border-pink-900/40">
                          <div className="flex items-center justify-between border-b border-pink-200/60 pb-2">
                            <h4 className="text-xs font-bold tracking-wide uppercase text-[#FF6584] flex items-center gap-1.5">
                              <Sparkles className="size-3.5 text-[#FF6584]" /> Product Option Variants
                            </h4>
                          </div>
                              ...
                              return (
                                <div key={attr.key} className="sm:col-span-2 space-y-2">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-[10px] text-[#FF6584] font-bold">Variant</span>
                                  </div>
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${isSelected
                                              ? "border-[#FF6584] bg-[#FF6584] text-white font-bold shadow-xs"
                                              : "border-border bg-background text-foreground hover:border-[#FF6584]/50"
                                              }`}
                                          >
                                            {opt}
                                          </button>
