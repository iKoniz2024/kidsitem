...
export default function ProductCard({ product, index, badge }) {
  ...
  return (
    <>
      ...
      >
        <div className={`group flex h-auto w-full flex-col overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-2xs transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:bg-slate-900 dark:border-slate-800 ${badgeConfig[effectiveBadge]?.ring ?? ""}`}>
          <Link href={`/product/${product._id}`} className="relative h-[180px] w-full overflow-hidden bg-[#E0F2FE] block shrink-0 p-2 flex items-center justify-center">
            ...
            {effectiveBadge && badgeConfig[effectiveBadge] && (
              <div className="absolute right-2 top-2 z-10">
                <Badge className="gap-1 text-[8px] sm:text-[9px] font-bold px-2 py-0.5 bg-[#FF6584] text-white border-none shadow-xs">
            ...
            {!effectiveBadge && product.stock <= 5 && product.stock > 0 && (
              <div className="absolute right-2 top-2 z-10">
                <Badge variant="secondary" className="text-[9px] font-semibold px-2 py-0.5 bg-pink-100 text-[#FF6584] border border-pink-200">
          ...
          <div className="flex flex-1 flex-col justify-between p-3 bg-white dark:bg-slate-900 shrink-0 gap-2">
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between gap-1">
                <span className="text-[9px] sm:text-[10px] text-slate-400 uppercase tracking-wider font-semibold truncate">
